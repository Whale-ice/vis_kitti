...
parse.add_argument('--pred', type=bool, default=False)
...
def get_objects(vis, index, pred=False):
    calib1 = get_calib(index)
    if pred:
        box_path = os.path.join(path, "pred", "{:06d}.txt".format(index))
    else:
...
def main(index):
    ...
    if args.pred:
        get_objects(vis, index, args.pred)
    get_objects(vis, index, pred=False)
    vis.run()
